...
namespace FenceHelper {
    ...
    public:
        ...
        Fence(CD3DX12AffinityCommandQueue* pQueue) :
            m_currentFenceValue(0), m_nodeMask(0)
        {
            DX::ThrowIfFailed(pQueue->QueryInterface(IID_PPV_ARGS(&m_queue)));

            ComPtr<CD3DX12AffinityDevice> device;
            DX::ThrowIfFailed(m_queue->GetDevice(IID_PPV_ARGS(&device)));

            DX::ThrowIfFailed(device->CreateFence(m_currentFenceValue, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_fence)));
            ...
            m_nodeMask = (1 << device->GetNodeCount()) - 1;
        }
        ...
        inline void Signal(UINT64 value = UnspecifiedFenceValue)
        {
            ...
            DX::ThrowIfFailed(m_queue->Signal(m_fence.Get(), m_currentFenceValue));
        }
        ...
        inline void GpuWait(UINT64 value = UnspecifiedFenceValue)
        {
            ...
            DX::ThrowIfFailed(m_queue->Wait(m_fence.Get(), value));
        }
        ...
        inline static void GpuWait(CD3DX12AffinityCommandQueue* pQueue, Fence* pFence, UINT64 value = UnspecifiedFenceValue)
        {
            ...
            pQueue->Wait(pFence->m_fence.Get(), value, true, pFence->m_nodeMask);
        }
    ...
    protected:
        ...
        inline void CpuWait(UINT64 fenceValue = UnspecifiedFenceValue)
        {
            ...
            {
                DX::ThrowIfFailed(m_fence->SetEventOnCompletion(fenceValue, m_fenceEvent));
